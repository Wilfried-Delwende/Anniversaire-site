...
function createBear(container) {
  ...
  function setPose(poseName) {
    const classesActuelles = el.className.split(' ').filter(function (nomClasse) {
      return nomClasse.indexOf('bear--pose-') !== 0 && nomClasse !== '';
    });
    classesActuelles.push('bear--pose-' + poseName);
    el.className = classesActuelles.join(' ');
  }
