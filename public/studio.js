...
import {
  ...
  collection, addDoc, updateDoc, deleteDoc, doc, writeBatch, getDocs,
...
loginForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    loginError.textContent = "Connexion impossible \u2014 code : " + codeErreur;
  }
});
...
/* ---------- Export de tout le contenu ---------- */
/*
   Cette fonction lit absolument tous les blocs de toutes les salles
   dans Firestore, les range par salle puis par ordre d'affichage, et
   construit un fichier texte lisible qui peut être envoyé directement
   dans la conversation pour la programmation du site.
*/

async function exportAllContent() {
  const exportButton = document.getElementById('exportBtn');
  const texteOriginalDuBouton = exportButton.textContent;
  exportButton.disabled = true;
  exportButton.textContent = 'Export\u2026';

  try {
    const snapshot = await getDocs(collection(db, 'blocs'));

    const tousLesBlocs = [];
    snapshot.forEach((docSnapshot) => {
      tousLesBlocs.push({ id: docSnapshot.id, ...docSnapshot.data() });
    });

    const blocsParSalle = {};
    tousLesBlocs.forEach((bloc) => {
      const numeroSalle = bloc.salle ?? 0;
      if (!blocsParSalle[numeroSalle]) {
        blocsParSalle[numeroSalle] = [];
      }
      blocsParSalle[numeroSalle].push(bloc);
    });

    Object.keys(blocsParSalle).forEach((numeroSalle) => {
      blocsParSalle[numeroSalle].sort(function (blocA, blocB) {
        return (blocA.ordre ?? 0) - (blocB.ordre ?? 0);
      });
    });

    let texteExporte = 'EXPORT DU CONTENU \u2014 SITE ANNIVERSAIRE\n';
    texteExporte += 'Généré le ' + new Date().toLocaleString('fr-FR') + '\n';

    ROOMS.forEach((salle) => {
      texteExporte += '\n\n========================================\n';
      texteExporte += 'SALLE ' + salle.id + ' \u2014 ' + salle.name + '\n';
      texteExporte += '========================================\n';

      const blocsDeCetteSalle = blocsParSalle[salle.id] || [];

      if (blocsDeCetteSalle.length === 0) {
        texteExporte += '(aucun contenu ajouté pour l\u2019instant)\n';
        return;
      }

      blocsDeCetteSalle.forEach((bloc, index) => {
        texteExporte += '\n--- Bloc ' + (index + 1) + ' [' + bloc.type + '] ';
        texteExporte += (bloc.etiquette || '(sans étiquette)') + ' ---\n';

        if (bloc.type === 'texte') {
          texteExporte += (bloc.contenu || '') + '\n';
        } else {
          texteExporte += 'Fichier : ' + (bloc.url || '(fichier manquant)') + '\n';
          if (bloc.legende) {
            texteExporte += 'Légende / message associé : ' + bloc.legende + '\n';
          }
        }
      });
    });

    const blobExport = new Blob([texteExporte.trim()], { type: 'text/plain;charset=utf-8' });
    const urlTelechargement = URL.createObjectURL(blobExport);

    const lienTelechargement = document.createElement('a');
    lienTelechargement.href = urlTelechargement;
    lienTelechargement.download = 'contenu-site-anniversaire.txt';
    document.body.appendChild(lienTelechargement);
    lienTelechargement.click();
    document.body.removeChild(lienTelechargement);
    URL.revokeObjectURL(urlTelechargement);
  } catch (erreurExport) {
    console.error(erreurExport);
    alert('Export impossible : ' + erreurExport.message);
  } finally {
    exportButton.disabled = false;
    exportButton.textContent = texteOriginalDuBouton;
  }
}
      ...
      if (file) {
        ...
        if (pickedType === 'photo') {
          ...
          try {
            ...
            console.error('Compression impossible, envoi du fichier original :', compressionError);
            fileToUpload = file;
          }
        }

        uploadProgress.classList.remove('hidden');
        mediaData = await uploadToCloudinary(fileToUpload, (pct) => {
...
/*
   Les photos prises avec un smartphone dépassent souvent la limite de
   10 Mo imposée par le plan gratuit de Cloudinary pour les images.
   Cette fonction redimensionne et recompresse la photo directement dans
   le navigateur, avant l'envoi, pour rester sous cette limite.

   Étapes :
   1. On charge le fichier original dans une image invisible.
   2. On calcule une nouvelle taille, si besoin, pour que le plus grand
      côté ne dépasse pas maxDimension pixels (les proportions de
      l'image sont conservées).
   3. On dessine cette image redimensionnée sur un canevas (canvas).
   4. On exporte ce canevas en JPEG avec le niveau de qualité choisi.
   5. On renvoie ce nouveau fichier compressé.

   Si quoi que ce soit se passe mal pendant ces étapes, on renvoie le
   fichier original tel quel, pour ne jamais bloquer l'envoi.
*/

function compressImage(originalFile, maxDimension, quality) {
  ...
  return new Promise(function (resolve) {
    ...
    image.onload = function () {
      ...
      const imageIsTooLarge = targetWidth > maxDimension || targetHeight > maxDimension;
      ...
      canvas.toBlob(
        function (blob) {
          ...
          const originalNameWithoutExtension = originalFile.name.replace(/\.\w+$/, '');
          const newFileName = originalNameWithoutExtension + '.jpg';
