...
import { CONTENT } from './js-content.js?v=4';
import { STATE, allerA } from './js-main.js?v=4';
import { createBear, createBearArmSilhouette } from './js-bear.js?v=4';

let ours = null;
let oursArrivee = false;
let salle1DejaDemarree = false;
...
function genererFeuillesCoeur(conteneur, nombre) {
  const couleurs = ['#e88fa0', '#f2b6c6', '#e8b262', '#f6d4de', '#d88fa8', '#c9587a'];
  conteneur.innerHTML = '';
  ...
  for (let i = 0; i < nombre; i++) {
    ...
    // Racine carrée d'un nombre aléatoire : donne une répartition bien
    // répartie sur TOUTE la surface du cœur (pas seulement le contour),
    // pour un feuillage dense et plein plutôt que des points épars.
    const remplissage = Math.sqrt(Math.random());

    const xCoeurMaths = 16 * Math.pow(Math.sin(t), 3);
    const yCoeurMaths = 13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t);

    // On inverse l'axe Y : les coordonnées mathématiques montent vers le
    // haut, celles de l'écran descendent vers le bas.
    const x = centreX + xCoeurMaths * echelle * remplissage + (Math.random() - 0.5) * 12;
    const y = centreY - yCoeurMaths * echelle * remplissage * 0.92 + (Math.random() - 0.5) * 12;
    ...
    const tailleAleatoire = 0.7 + Math.random() * 0.7;
    feuille.style.transform = 'scale(' + tailleAleatoire.toFixed(2) + ')';
...
async function demarrerSalle1() {
  if (salle1DejaDemarree) return;
  salle1DejaDemarree = true;
...
async function phaseVeilleuse() {
  ...
  halo.classList.add('est-allumee');
  lampeBtn.classList.add('est-allumee');
  document.getElementById('couchePiece').classList.add('est-eclairee');
...
async function phaseExplosion() {
  ...
  camera.style.transform = 'translate(-50%, 40dvh) scale(1.1)';
  camera.style.opacity = '1';

  await pause(120);
  montrerReplique(CONTENT.salle1.criEnvol);

  // Un tout petit temps d'"écrasement" avant le décollage, pour donner
  // l'impression d'un ressort qui se détend violemment vers le haut.
  camera.style.transition = 'transform 0.12s ease-in';
  camera.style.transform = 'translate(-50%, 46dvh) scale(1.25, 0.8)';
  await pause(120);

  monde.classList.add('va-vite');
  monde.style.transform = 'translateY(0)';

  // Décollage : trajectoire longue, bien au-dessus de l'écran, avec un
  // rétrécissement marqué qui donne la sensation de vitesse et d'altitude.
  camera.style.transition = 'transform 0.85s cubic-bezier(0.5, 0, 0.75, 0.15), opacity 0.4s ease 0.5s';
  camera.style.transform = 'translate(-50%, -160dvh) scale(0.25)';
...
async function phaseChute() {
  ...
  camera.style.transition = 'none';
  camera.style.transform = 'translate(-50%, -150dvh) scale(0.3)';
  camera.style.opacity = '1';
  ...
  camera.style.transition = 'transform 0.7s cubic-bezier(0.5, 0, 1, 0.4)';
...
async function phaseArbre() {
  ...
  await pause(1000);
  genererFeuillesCoeur(feuillage, 150);
